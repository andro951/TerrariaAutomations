...
namespace TerrariaAutomations.Tiles.TileEntities {
	internal class BlockPlacerTE : ModTileEntity, IUseChestIndicators {
		...
		public override bool IsTileValidForEntity(int x, int y) {
			...
			return modTile is Tiles.BlockPlacer;
		}
		...
		public override void Update() {
			...
			if (Main.netMode == NetmodeID.Server) {
				...
				if (sendCounter >= sendCounterReset) {
					sendCounter = 0;
					BlockPlacer.SendChestDatas(x, y);
				}
			}
			...
			ModTile modTile = ModContent.GetModTile(Main.tile[x, y].TileType);
			if (modTile is Tiles.BlockPlacer blockPlacerModTile)
				blockPlacerModTile.HitWire(x, y);
		}
	}
}
